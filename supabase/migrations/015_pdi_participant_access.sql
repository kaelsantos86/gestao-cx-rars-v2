create or replace function public.get_pdi_participant_record(raw_token text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  return jsonb_build_object(
    ...
    'shared_context', jsonb_build_object(
      'cycleType', record_row.payload->>'cycleType',
      'contextAndRole', record_row.payload->>'contextAndRole',
      'currentMoment', record_row.payload->>'currentMoment',
      'strengthsToPreserve', record_row.payload->>'strengthsToPreserve',
      'aspiration', record_row.payload->>'aspiration',
      'developmentDirection', record_row.payload->>'developmentDirection',
      'priorities', coalesce(record_row.payload->'priorities', '[]'::jsonb)
    ),
...
revoke all on function public.get_pdi_participant_record(text) from public;
grant execute on function public.get_pdi_participant_record(text) to anon, authenticated;
