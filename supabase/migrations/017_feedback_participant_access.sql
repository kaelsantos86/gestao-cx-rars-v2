create or replace function public.get_feedback_participant_record(raw_token text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  return jsonb_build_object(
    ...
    'shared_context', jsonb_build_object(
      'feedbackFlow', record_row.payload->>'feedbackFlow',
      'recognitionMode', record_row.payload->>'recognitionMode',
      'shareableContext', record_row.payload->>'shareableContext'
    ),
...
revoke all on function public.get_feedback_participant_record(text) from public;
grant execute on function public.get_feedback_participant_record(text) to anon, authenticated;
